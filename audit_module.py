...
def render_forensic_alerts(row, df_master):
    ...
    def alert_box(label, status, icon="✅"):
        color = "#39d353" if status == "NORMAL" else "#f85149"
        bg = "rgba(57, 211, 83, 0.1)" if status == "NORMAL" else "rgba(248, 81, 73, 0.1)"
        st.markdown(f"""
            <div style="padding: 10px; border-radius: 5px; background: {bg}; border-left: 5px solid {color}; margin-bottom: 10px;">
                <span style="font-size: 18px;">{icon}</span>
                <b style="color: {color}; margin-left: 10px;">{label}</b><br>
                <small style="color: #8b949e; margin-left: 30px;">Estado: {status}</small>
            </div>
        """, unsafe_allow_html=True)

    # Score Gauge
    score_color = "#39d353" if issuer_score < 30 else ("#ff9f1c" if issuer_score < 70 else "#f85149")
    st.markdown(f"""
        <div style="text-align: center; padding: 15px; background: rgba(88, 166, 255, 0.05); border-radius: 10px; border: 1px solid #30363d; margin-bottom: 20px;">
            <div style="font-size: 10px; color: #8b949e; text-transform: uppercase; letter-spacing: 2px;">Índice de Riesgo Emisor</div>
            <div style="font-size: 32px; font-weight: bold; color: {score_color};">{issuer_score:.1f}%</div>
        </div>
    """, unsafe_allow_html=True)
...
def render_invoice_html(row, df_conceptos):
    ...
    html_template = f"""
    <div style="font-family: 'Inter', sans-serif; max-width: 900px; margin: 0 auto; background: white; color: #333; box-shadow: 0 4px 6px rgba(0,0,0,0.1); border: 1px solid #e0e0e0;">
        <div style="background-color: #0d1117; color: #58a6ff; padding: 30px; display: flex; justify-content: space-between; border-bottom: 4px solid #58a6ff;">
            <div>
                <h1 style="margin: 0; font-size: 24px; font-weight: bold;">ANÁLISIS DE CFDI</h1>
                <p style="margin: 5px 0 0; opacity: 0.8; font-size: 12px; font-family: 'JetBrains Mono';">{row.get('uuid', 'N/A')}</p>
            </div>
            <div style="text-align: right;">
                <h3 style="margin: 0;">{row.get('emisor_nombre', 'N/A')}</h3>
                <p style="margin: 2px 0; font-size: 12px;">RFC: {row.get('emisor_rfc', 'N/A')}</p>
                <p style="margin: 0; font-size: 12px;">Fecha: {row.get('fecha_emision', 'N/A')}</p>
            </div>
        </div>
        
        <div style="padding: 20px; background-color: #f8f9fa; border-bottom: 1px solid #eee;">
            <p style="margin: 0; color: #58a6ff; font-weight: bold; font-size: 10px; text-transform: uppercase;">Receptor</p>
            <h2 style="margin: 0; font-size: 18px;">{row.get('receptor_nombre', 'N/A')}</h2>
            <p style="margin: 2px 0; font-size: 12px;">RFC: {row.get('receptor_rfc', 'N/A')}</p>
        </div>

        <div style="padding: 20px;">
            <table style="width: 100%; border-collapse: collapse; font-size: 12px;">
                <thead>
                    <tr style="color: #0d1117; background: #f0f0f0;">
                        ...
                        <th style="text-align: right; padding: 8px;">P. UNITARIO</th>
        ...
        <div style="padding: 0 20px 30px; display: flex; justify-content: flex-end;">
            <table style="width: 250px; font-size: 13px; border-collapse: collapse;">
                <tr><td style="padding: 5px; text-align: right;">Subtotal:</td><td style="padding: 5px; text-align: right;">${fmt(row.get('subtotal', 0))}</td></tr>
                <tr><td style="padding: 5px; text-align: right;">IVA:</td><td style="padding: 5px; text-align: right;">${fmt(row.get('calc_iva', 0))}</td></tr>
                <tr><td style="padding: 5px; text-align: right; border-top: 2px solid #0d1117; font-weight: bold;">TOTAL CFDI:</td><td style="padding: 5px; text-align: right; border-top: 2px solid #0d1117; font-weight: bold;">${fmt(row.get('total', 0))}</td></tr>
            </table>
        </div>
    </div>
    """
