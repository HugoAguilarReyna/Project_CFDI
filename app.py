...
def render_futuristic_header():
    ...
    /* Removed sticky header CSS. Normal content flow. */
    ...
    .neon-title {
        ...
        letter-spacing: 5px;
        text-align: center;
        background: linear-gradient(90deg, #58a6ff, #00d4ff, #58a6ff);
    ...
    .stat-delta {
        font-family: 'Inter', sans-serif;
        font-size: 12px;
        color: #39d353; /* Green arrows like in the image */
        font-weight: 600;
    }

    /* 9. Sticky Header Logic */
    /* Update: Reset main container padding */
    .block-container {
    ...
    /* CRITICAL: Force the direct parent of the sticky header to have no height constraint */
    ...
    /* Target the ACTUAL wrapper div created by st.container() */
    ...
    /* Ensure the iframe (menu) within it is visible */
    iframe {
...
def plot_dark_histogram(df, x_col, y_col, color_col, title):
    fig = px.bar(df, x=x_col, y=y_col, color=color_col, 
                 color_discrete_sequence=['#00f2ff', '#0047ab', '#5200cc'], # Cyan, Blue, Purple
                 title=title)
    fig.update_layout(
...
if not st.session_state.authenticated:
    ...
    st.markdown("""
        <div style="text-align: center; margin-top: 100px;">
            <h1 style="font-family: 'JetBrains Mono'; font-size: 50px; color: #58a6ff;">SECURE ACCESS</h1>
            <p style="color: #8b949e;">RESTRICTED AREA // FORENSIC SYSTEM</p>
        </div>
    """, unsafe_allow_html=True)
...
if st.session_state.authenticated:
    ...
    with st.container():
        ...
        selected_module = option_menu(
            ...
            icons=["speedometer", "search", "shield-exclamation", "file-earmark-check", "sliders"],
            menu_icon="cast",
            default_index=0,
...
# --- GLOBAL HIGH-RISK ALERTS (BANNER) ---
if not df.empty:
    # Adjusted for demo: Round amount OR High Value (>500k)
    alert_mask = (df['total'] > 0) & ((df['total'] % 100 == 0) | (df['total'] > 500000))
    high_risk_cfdis = df[alert_mask]
    
    if not high_risk_cfdis.empty:
        alert_text = "  |  ".join([
            f"⚡ ALERTA DE RIESGO: CFDI {r['uuid'][:8]} - {r['emisor_rfc']} (${r['total']:,.2f})"
            for _, r in high_risk_cfdis.head(5).iterrows()
        ])
        st.markdown(f"""
            <div style="background: rgba(248, 81, 73, 0.05); border-top: 1px solid rgba(248, 81, 73, 0.3); border-bottom: 1px solid rgba(248, 81, 73, 0.3); padding: 8px 0; overflow: hidden; white-space: nowrap; margin-bottom: 20px;">
                <div style="display: inline-block; animation: marquee 40s linear infinite;">
                    <span style="color: #f85149; font-family: 'JetBrains Mono', monospace; font-size: 13px; font-weight: bold; text-transform: uppercase;">
...
# --- DATA FILTERING LOGIC (MIGRATED FROM SIDEBAR) ---
# Since we removed the sidebar, we need a place for filters.
# We will place them in an expander at the top IF Config is not selected, or per module.
# For now, let's keep it simple: A Global Filter Bar above content.

if selected_module != "Configuración":
    with st.expander("🔍 FILTROS GLOBALES Y SEGMENTACIÓN", expanded=False):
        c1, c2, c3, c4 = st.columns(4)
        with c1:
             # Calculate Ventas Netas safely
            df['ventas_netas_calc'] = (df['subtotal'] + df['calc_iva']) - (df['calc_retenciones'] + df['descuento'])
            if not df.empty:
                min_net = float(df['ventas_netas_calc'].min())
                max_net = float(df['ventas_netas_calc'].max())
            else:
                min_net, max_net = 0.0, 1000.0
            if min_net == max_net: max_net += 0.01
            sales_range = st.slider("Rango Ventas Netas", min_net, max_net, (min_net, max_net))
        ...
        with c3:
            ...
            time_agg_label = st.radio("Agrupación Temporal", options=list(time_agg_map.keys()), index=2, horizontal=True) 
    ...
    # --- APPLY FILTERS ---
    mask = (df['ventas_netas_calc'] >= sales_range[0]) & (df['ventas_netas_calc'] <= sales_range[1])
    ...
    df_filtered = df.copy() # No filters on config page initially
...
def render_stat_element(label, value, sub_label, color="#ffffff"):
    st.markdown(f"""
...
if selected_module == "Configuración":
    ...
    # Sub-navigation for Strategy
    sub_tab1, sub_tab2, sub_tab3 = st.tabs(["KPIs OPERATIVOS", "ANÁLISIS ESTRUCTURAL", "TENDENCIAS"])
    ...
    with sub_tab1:
        ...
        with s1: render_stat_element("Monto Máximo", f"${df_filtered['total'].max():,.2f}", "Peak Value", "#ffffff")
        with s2: render_stat_element("Desviación Est.", f"${df_filtered['total'].std():,.2f}", "Sigma Variance", "#ffffff")
        with s3: render_stat_element("Rango Operativo", f"${df_filtered['total'].max() - df_filtered['total'].min():,.2f}", "Full Spread", "#ffffff")
        with s4: render_stat_element("Promedio", f"${avg:,.2f}", "Mean Density", "#ffffff")
        ...
        with st.container():
            if not df_filtered.empty:
                ...
                q_vals = df_filtered['total'].quantile([0.2, 0.4, 0.6, 0.8]).values
...
# --- VIEW: RISK CENTER ---
    ...
    if df_filtered.empty:
        ...
        risk_tab1, risk_tab2 = st.tabs(["DETECCIÓN DE ANOMALÍAS", "RANKING DE RIESGO PROVEEDORES"])
        ...
        with risk_tab2:
            ...
            with c1:
                ...
                st.dataframe(
                    ...
                    column_config={
                        # "risk_score": st.column_config.ProgressColumn("Risk Index", format="%.0f", min_value=0, max_value=100, color="red"),
                        "risk_score": st.column_config.ProgressColumn("Risk Index", format="%.0f", min_value=0, max_value=100),
...
# --- VIEW: COMPLIANCE ---
     ...
     tab_iva, tab_flujo = st.tabs(["🛡️ AUDITORÍA DE TASAS (IVA)", "💰 INTEGRIDAD DE FLUJO (PPD vs REP)"])
...
# --- VIEW: SETTINGS (CONFIGURACIÓN) ---
    ...
    t1, t2 = st.tabs(["⚙️ General", "🔮 Modelos AI"])
